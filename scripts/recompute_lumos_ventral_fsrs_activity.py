...
def main():
    ...
    lu=lu.merge(lm[['unit_key','analyzer_path','after_trough_to_peak_duration_ms','after_waveform_asymmetry','after_post_trough_rebound_slope_uV_per_ms','after_rep50_recovery_slope_uV_per_ms']],on='unit_key',how='inner')
    cy=cy.merge(cm[['unit_key','after_trough_to_peak_duration_ms','after_waveform_asymmetry','after_post_trough_rebound_slope_uV_per_ms','after_rep50_recovery_slope_uV_per_ms']],on='unit_key',how='left')
    ...
    for ax,(m,label) in zip(axs,metrics):
        ...
        for i,c in enumerate(['FS','RS']):
            v=pd.to_numeric(out.loc[out.rs_fs_class.eq(c),m],errors='coerce').dropna(); vals.append(v)
