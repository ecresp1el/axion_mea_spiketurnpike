#!/usr/bin/env python
"""Prepare and optionally submit Wave A/B/C representative scoring."""
...
DEFAULT_WAVE0_ROOT = DEFAULT_JOB_DIR / "representative_units_20260709_wave0_20260709_162201"
...
def main() -> int:
    ...
    output_dir = (
        ...
        else DEFAULT_JOB_DIR / f"representative_units_{args.date_label}_abc_scoring_{timestamp}"
    )
    ...
    command_file.write_text(
        "\n".join(
            [
                ...
                "set -u",
                (
                    "python scripts/score_representative_unit_candidates.py "
                    f"--input-index {shell_quote(args.input_index.expanduser().resolve())} "
    ...
    manifest = {
        ...
        "input_index": str(args.input_index.expanduser().resolve()),
    ...
    if args.submit:
        ...
        (output_dir / "submitted_jobs.tsv").write_text(
            ...
            f"{datetime.now().isoformat(timespec='seconds')}\trepresentative_abc_scoring_{timestamp}\t{job_id}\t{output_dir}\t{command_file}\t{submitted_sbatch}\n",
            encoding="utf-8",
        )
        print(f"submitted_job_id={job_id}")

    (output_dir / "abc_scoring_prepare_manifest.json").write_text(
