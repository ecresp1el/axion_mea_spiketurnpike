...
def inspect_input(input_dir: Path, source_xml: Path | None, source_msrd: Path | None = None) -> dict:
    ...
    if bool(model_match[3]) != has_ref or count != (59 if has_ref else 60):
...
def make_params(template: dict, inspected: dict, probe_path: Path, session_name: str) -> dict:
    ...
    neighborhood = inspected["pitch_um"] * 350.0 / 300.0
    ks["sorter"].update({"Th_universal": 5, "Th_learned": 8, "do_CAR": False,
        ...
        "dmin": inspected["pitch_um"], "dminx": inspected["pitch_um"],
...
def prepare(input_dir: Path, source_xml: Path | None, project: Path, run_id: str, template_path: Path,
            *, source_msrd: Path | None = None) -> dict:
    ...
    inspected = inspect_input(input_dir, source_xml, source_msrd)
    prep = inspected["preparation"]
    template = json.loads(template_path.read_text())
    minimum = float(template["preprocessing"]["min_preprocessing_duration"])
    if prep["sample_count"] / prep["sampling_frequency_hz"] < minimum:
    ...
    probe = Probe(ndim=2, si_units="um", name=inspected["model"], manufacturer="Multi Channel Systems")
    probe.set_contacts(np.array([[c["x_um"], c["y_um"]] for c in inspected["channels"]]),
                       shapes="circle", shape_params={"radius": inspected["contact_radius_um"]})
    ...
    manifest = prep | {"analysis_kind": "mcs_aind_single_mea_recording", "recording_id": recording_id,
        ...
        "decisions": {"source_counts": "unchanged int32", "source_filter_state": "raw acquisition; no claim about analog frontend",
            ...
            "waveform_window": "nt=31 retained, 3.1 ms at 10 kHz", "spatial_radius": "CytoView 350/300 times verified MCS pitch",
...
def main() -> None:
    ...
    print(json.dumps(prepare(args.input_dir, args.source_xml, args.project_root, args.run_id,
                             args.params_template, source_msrd=args.source_msrd), indent=2))
