#!/usr/bin/env python3
"""Prepare and optionally submit the CytoView RS/FS feature-separability audit."""
...
def main() -> int:
    ...
    output_dir = args.output_root.expanduser().resolve() / f"cytoview_rsfs_feature_separability_20260710_{timestamp}"
    repro_dir = output_dir / "repro"
    logs_dir = output_dir / "logs"
    repro_dir.mkdir(parents=True, exist_ok=False)
    logs_dir.mkdir(parents=True, exist_ok=True)
    command = [
        "python", "scripts/analyze_cytoview_rsfs_feature_separability.py",
        "--output-dir", str(output_dir),
        "--fs-cutoff-ms", str(args.fs_cutoff_ms),
        "--export-formats", args.export_formats,
    ]
    ...
    shutil.copy2(REPO_ROOT / "scripts" / "analyze_cytoview_rsfs_feature_separability.py", repro_dir / "analyze_cytoview_rsfs_feature_separability.py")
    sbatch_path = repro_dir / "submitted_job.sbatch"
    sbatch_path.write_text(
        f"""#!/usr/bin/env bash
#SBATCH --job-name=rsfs_feature_audit
#SBATCH --account=parent0
#SBATCH --partition=standard
#SBATCH --cpus-per-task=2
#SBATCH --mem=16G
#SBATCH --time=01:00:00
#SBATCH --output={logs_dir}/feature_audit_%j.out
#SBATCH --error={logs_dir}/feature_audit_%j.err
...
""",
    ...
    if args.submit:
        result = subprocess.run(["sbatch", "--parsable", str(sbatch_path)], capture_output=True, text=True, check=False)
