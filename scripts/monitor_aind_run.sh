...
while true; do
  ...
  newest_out="$(newest_file "${work_dir}" ".command.out" || true)"
  newest_err="$(newest_file "${work_dir}" ".command.err" || true)"
  newest_log="$(newest_file "${work_dir}" ".command.log" || true)"
  print_file_tail "newest .command.out" "${newest_out}" 35
  if [[ -n "${newest_err}" && -s "${newest_err}" ]]; then
    print_file_tail "newest non-empty .command.err" "${newest_err}" 35
  else
    echo "[monitor] newest .command.err: no non-empty stderr yet"
  fi
  print_file_tail "newest .command.log" "${newest_log}" 20
