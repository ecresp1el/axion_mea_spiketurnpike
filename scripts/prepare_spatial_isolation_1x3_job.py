#!/usr/bin/env python
"""Prepare and optionally submit spatial-isolation 1x3 panel rendering."""
...
def main() -> int:
    ...
    output_dir = (
        ...
        else DEFAULT_JOB_DIR / f"representative_units_{args.date_label}_spatial_isolation_1x3_{timestamp}"
    )
    ...
    command_file.write_text(
        "\n".join(
            [
                ...
                "set -u",
                (
                    "python scripts/plot_spatial_isolation_1x2_panels.py "
                    f"--selection-root {shell_quote(args.selection_root.expanduser().resolve())} "
                    ...
                    f"--units-per-panel {args.units_per_panel} "
                    f"--min-spikes {args.min_spikes} "
    ...
    manifest = {
        ...
        "selection_root": str(args.selection_root.expanduser().resolve()),
    ...
    if args.submit:
        ...
        (output_dir / "submitted_jobs.tsv").write_text(
            ...
            f"{datetime.now().isoformat(timespec='seconds')}\tspatial_isolation_1x3_{timestamp}\t{job_id}\t{output_dir}\t{command_file}\t{submitted_sbatch}\n",
            encoding="utf-8",
        )
        print(f"submitted_job_id={job_id}")

    (output_dir / "spatial_isolation_1x3_prepare_manifest.json").write_text(
