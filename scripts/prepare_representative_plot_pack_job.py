#!/usr/bin/env python
"""Prepare and optionally submit representative-unit plot-pack rendering."""
...
def main() -> int:
    ...
    output_dir = (
        ...
        else DEFAULT_JOB_DIR / f"representative_units_{args.date_label}_plot_pack_{timestamp}"
    )
    ...
    command_file.write_text(
        "\n".join(
            [
                ...
                "set -u",
                (
                    "python scripts/plot_representative_unit_pack_from_manifests.py "
                    ...
                    f"--limit-per-table {args.limit_per_table}"
                ),
                "",
            ]
        ),
    ...
    if args.submit:
        ...
        (output_dir / "submitted_jobs.tsv").write_text(
            ...
            f"{datetime.now().isoformat(timespec='seconds')}\trepresentative_plot_pack_{timestamp}\t{job_id}\t{output_dir}\t{command_file}\t{submitted_sbatch}\n",
            encoding="utf-8",
        )
        print(f"submitted_job_id={job_id}")

    (output_dir / "plot_pack_prepare_manifest.json").write_text(
