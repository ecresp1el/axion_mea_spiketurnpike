#!/usr/bin/env python
"""Prepare and optionally submit spatial-isolation 1x2 panel rendering."""
...
def main() -> int:
    ...
    output_dir = (
        ...
        else DEFAULT_JOB_DIR / f"representative_units_{args.date_label}_spatial_isolation_1x2_{timestamp}"
    )
    ...
    if args.submit:
        ...
        (output_dir / "submitted_jobs.tsv").write_text(
            ...
            f"{datetime.now().isoformat(timespec='seconds')}\tspatial_isolation_1x2_{timestamp}\t{job_id}\t{output_dir}\t{command_file}\t{submitted_sbatch}\n",
            encoding="utf-8",
        )
        print(f"submitted_job_id={job_id}")

    (output_dir / "spatial_isolation_1x2_prepare_manifest.json").write_text(
